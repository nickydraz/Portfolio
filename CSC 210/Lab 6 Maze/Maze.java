...
public class Maze {
	 ...
	 * 
	 */
	public boolean move(int myRow, int myCol)
	{
		
		//Check if the current move is out of bounds
		if (myRow > mazeArray.length - 1 || myRow < 0 
				|| myCol > mazeArray.length - 1|| myCol < 0)
			return false;
		...
		//If the space is a wall, return false.
		if (mazeArray[myRow][myCol] == 'X')
			return false;
		
		//If the space has already been traversed, return false.
		//This prevents getting stuck in a loop of spaces
		if (mazeArray[myRow][myCol] == '*' || mazeArray[myRow][myCol] == '.')
			return false;
	
		//Checks if the space is an invalid space
		//that was found when reading in the text file
		if (mazeArray[myRow][myCol] == '@')
			return false;
		...
		//North
		if (move(myRow - 1, myCol))
			return true;
		
		//East
		if (move(myRow, myCol + 1))
			return true;
	...
	//*** GUI VERSION of move method -- moveGUI ******
	public boolean moveGUI(int myRow, int myCol)
	{
		...
		//North
		if (moveGUI(myRow - 1, myCol))
			return true;
		
		//East
		if (moveGUI(myRow, myCol + 1))
			return true;
